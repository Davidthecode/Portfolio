        ...
        <main className="flex flex-col mt-12 mb-40">
            ...
            <section className="flex flex-col gap-6 mt-16">
                <div className="flex flex-col items-center space-y-2">
                    ...
                    <h1 className="text-5xl font-bold text-center">Check out my latest work</h1>
                    <p className="text-center text-xl opacity-50">I've worked on a variety of projects, from simple websites to complex web applications. Here are a few of my favorites.</p>
                </div>
                <div className="flex flex-col justify-center flex-wrap gap-4 sm:gap-3 sm:flex-row items-center opacity-80 mt-3">
                    {projects.map((project) => (
                        <article
                            className="group flex flex-col gap-2 border border-gray-300 border-opacity-70 hover:shadow-sm px-4 py-3 rounded-md w-full sm:w-[49%] h-fit sm:h-[18rem]"
                            key={project.link}
                        >
                            <a href={project.link} target="_blank" rel="noopener noreferrer">
                                <div>
                                    ...
                                    <p className="text-xs opacity-70 mt-2">{project.about}</p>
                                </div>
                            </a>
                        </article>
                    ))}
            ...
            <section className="text-xl font-semibold mt-8 flex flex-col space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-2">
                    {skills.map((skill) => (
                        <div className="py-[1px] px-[10px] rounded-md bg-black text-white text-[11px] w-fit text-opacity-">
                            {skill}
                        </div>
                    ))}
                </div>
